...
class PartitionDagLazyTest {
	...
	void initializeNeo4j() throws IOException {
		...
		this.embeddedDatabaseServer = Neo4jBuilders.newInProcessBuilder()
			.withProcedure(PartitionDagLazy.class)
	...
	void partitionSimple() {
		...
		try (var driver = GraphDatabase.driver(embeddedDatabaseServer.boltURI(), driverConfig);
			...
			var result = session.run("MATCH (n:SimpleNode {id: 'bc526bbb-d43b-4ced-a87d-4f16bcb415e8'}) WITH collect(n) AS nodesList CALL hanse.partition.lazy(nodesList, 'HAS_SUCCESSOR', 3) YIELD maxLevel, visitedStamp RETURN maxLevel, visitedStamp").single();
	...
	void partitionSimpleShouldStopAtMaxDepth() {
		...
		try (var driver = GraphDatabase.driver(embeddedDatabaseServer.boltURI(), driverConfig);
			...
			var result = session.run("MATCH (n:SimpleNode {id: 'bc526bbb-d43b-4ced-a87d-4f16bcb415e8'}) WITH collect(n) AS nodesList CALL hanse.partition.lazy(nodesList, 'HAS_SUCCESSOR', 1) YIELD maxLevel, visitedStamp RETURN maxLevel, visitedStamp").single();
	...
	void partitionShouldTraverseSubgraph() {
		...
		try (var driver = GraphDatabase.driver(embeddedDatabaseServer.boltURI(), driverConfig);
			...
			var result = session.run("MATCH (n:Node {id: 'a8e3f624-cdce-40b3-be2d-3acf138453f8'}) WITH collect(n) AS nodesList CALL hanse.partition.lazy(nodesList, 'HAS_SUCCESSOR', 10) YIELD maxLevel, visitedStamp RETURN maxLevel, visitedStamp").single();
			var visitedStamp = result.get("visitedStamp").asString();
	...
	void partitionShouldTraverseEntireGraphOneStartNode() {
		...
		try (var driver = GraphDatabase.driver(embeddedDatabaseServer.boltURI(), driverConfig);
			...
			var result = session.run("MATCH (n:Node {id: '3008d7cc-8e6c-4564-903f-d8f95ad0fdea'}) WITH collect(n) AS nodesList CALL hanse.partition.lazy(nodesList, 'HAS_SUCCESSOR', 10) YIELD maxLevel, visitedStamp RETURN maxLevel, visitedStamp").single();
			var visitedStamp = result.get("visitedStamp").asString();
	...
	void partitionShouldTraverseEntireGraphMultipleStartNodes() {
		...
		try (var driver = GraphDatabase.driver(embeddedDatabaseServer.boltURI(), driverConfig);
			...
			var result = session.run("MATCH (n:Node) WHERE n.id='3aeb7e39-3ab8-42f1-b257-3521486eb6fb' OR n.id='a8e3f624-cdce-40b3-be2d-3acf138453f8' WITH collect(n) AS nodesList CALL hanse.partition.lazy(nodesList, 'HAS_SUCCESSOR', 10) YIELD maxLevel, visitedStamp RETURN maxLevel, visitedStamp").single();
			var visitedStamp = result.get("visitedStamp").asString();
